...
export default function ActiveSessionPage() {
	...
	const climbsBySession = useClimbsBySession(parseInt(sessionId));

	console.log(`session data: ${JSON.stringify(climbsBySession?.data)}`);

	if (!climbsBySession?.data || !climbsBySession?.data?.length) {
		return (
			<View>
				<Text>Error</Text>
				<Text>
		...
		return (
			<ScrollView contentContainerStyle={styles.container}>
				...
				<View style={styles.climbListContainer}>
					{climbsBySession?.data?.map((climb, index) => (
